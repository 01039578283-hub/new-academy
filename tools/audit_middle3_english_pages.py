...
HUB = SUBJECT_ROOT / "중3영어학원"
MIDDLE2_HUB = SUBJECT_ROOT / "중2영어학원"
...
ENGLISH_INTENT_GROUPS = {
    "교과서·학교 자료": re.compile(r"교과서|학교\s*프린트|학교\s*자료|내신"),
    "어휘": re.compile(r"어휘|단어|품사"),
    "문법": re.compile(r"문법|어순|동사\s*형태|문장\s*구조"),
    "독해": re.compile(r"독해|지문|문단|근거\s*문장"),
    "서술형·쓰기": re.compile(r"서술형|영작|쓰기|답안\s*수정|문장\s*작성"),
}
FORBIDDEN_PATTERNS = {
    # 영어 해석 과정의 '한국어'는 국어 과목 언급이 아니므로 제외합니다.
...
def meta_values(text: str, key: str, attribute: str = "name") -> list[str]:
    values: list[str] = []
...
def visible_faq(text: str) -> list[tuple[str, str]]:
    ...
    for detail in re.findall(r"<details\b[^>]*>(.*?)</details>", section, flags=re.I | re.S):
        result.append(
            (
                strip_tags(first_match(r"<summary\b[^>]*>(.*?)</summary>", detail, re.I | re.S)),
                strip_tags(first_match(r"<p\b[^>]*>(.*?)</p>", detail, re.I | re.S)),
            )
        )
    return result
...
def visible_related_urls(text: str, base: str) -> list[str]:
    ...
    return [
        normalize_url(attrs(tag).get("href", ""), base)
...
def check_head(text: str, expected_title: str, expected_url: str, errors: list[str], label: str) -> None:
    ...
    canonicals = [normalize_url(value, expected_url) for value in canonical_values(text) if value]
    if canonicals != [normalize_url(expected_url)]:
        errors.append(f"{label}: canonical 불일치 ({canonicals})")
    og_urls = [normalize_url(value, expected_url) for value in meta_values(text, "og:url", "property") if value]
...
def main() -> int:
    ...
    if HUB.exists():
        ...
        actual_dirs = set()
        hub_text = ""
        errors.append(f"중3 영어 허브 폴더 없음: {HUB}")
    ...
    card_urls: list[str] = []
    if hub_text:
        check_head(hub_text, CATEGORY_LABEL, hub_url, errors, "허브")
        ...
        hub_breadcrumbs = typed_nodes(hub_blocks, "BreadcrumbList")
        if len(hub_breadcrumbs) != 1 or json_breadcrumb(hub_breadcrumbs[0])[0] != expected_hub_breadcrumb:
        ...
        matching_itemlists = [
            node
            for node in typed_nodes(hub_blocks, "ItemList")
            if set(itemlist_urls(node, hub_url)) == expected_urls
        ]
    ...
    for local in locals_:
        ...
        check_head(text, expected_title, url, errors, local)

        missing_sections = [name for name in sorted(REQUIRED_SECTIONS) if not section_html(text, name)]
        if missing_sections:
            errors.append(f"{local}: 영어 상세 필수 섹션 누락 ({missing_sections})")

        descriptions = meta_values(text, "description")
        description = clean_text(descriptions[0]) if len(descriptions) == 1 else ""
        ...
        if breadcrumb_labels(text) != expected_breadcrumb:
            errors.append(f"{local}: 화면 Breadcrumb 불일치 ({breadcrumb_labels(text)})")
        ...
        schema_faq = json_faq(faq_nodes[0]) if len(faq_nodes) == 1 else []
        if len(faq_nodes) != 1:
        ...
        schema_related = itemlist_urls(related_itemlists[0], url) if len(related_itemlists) == 1 else []
        ...
        if middle2_url not in visible_related:
            errors.append(f"{local}: 같은 동네 중2 영어학원 관련 링크 누락")
        if not (MIDDLE2_HUB / local / "index.html").exists():
            errors.append(f"{local}: 중2 영어학원 링크 대상 페이지 없음")
        ...
        if len(body) < 1800:
            errors.append(f"{local}: 영어 원고·보강 영역이 너무 짧음 ({len(body)}자)")
        for intent, pattern in ENGLISH_INTENT_GROUPS.items():
            if not pattern.search(body):
                errors.append(f"{local}: 영어 학습 의도 필수 요소 누락 ({intent})")
    ...
    missing_sitemap = ({normalize_url(hub_url)} | expected_urls) - set(sitemap_urls)
    if missing_sitemap:
        errors.append(f"중3 영어 사이트맵 URL 누락 {len(missing_sitemap)}개")
