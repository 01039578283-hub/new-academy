...
class AuditConfig:
    ...
    previous_label: str
    intent_groups: dict[str, Pattern[str]]
    forbidden_patterns: dict[str, Pattern[str]]
    min_manuscript_length: int = 1900
    faq_count: int = 7


ENGLISH_CONFIG = AuditConfig(
    category="고2영어학원",
    label="고2 영어학원",
    subject="영어",
    grade="고2",
    previous_category="고1영어학원",
    previous_label="고1 영어학원",
    intent_groups={
        "고2 내신 심화": re.compile(r"고2|고등학교\s*2학년|고교\s*내신|고등\s*내신|내신\s*범위"),
        "모의고사": re.compile(r"모의고사|전국연합|학력평가|모의평가"),
        "어휘": re.compile(r"어휘|단어|품사"),
        "구문·문법": re.compile(r"구문|문장\s*구조|문법|어순|동사\s*형태"),
        "독해·문항 근거": re.compile(r"독해|지문|문단|빈칸|순서|삽입|근거\s*문장"),
...
def canonical(config: AuditConfig, local: str | None = None) -> str:
    parts = ["과목별학원", config.category]
    if local:
        parts.append(local)
    return DOMAIN + "/" + "/".join(quote(part, safe="") for part in parts) + "/"


def previous_canonical(config: AuditConfig, local: str) -> str:
    parts = ["과목별학원", config.previous_category, local]
    return DOMAIN + "/" + "/".join(quote(part, safe="") for part in parts) + "/"
...
def verify_facts(
    ...
    grades = split_items(shared.row_value(row, f"가능학년\n({config.subject})"))
    schools = split_items(shared.row_value(row, "타깃학교\n(고)"))

    normalized_text = re.sub(r"\s+", " ", shared.strip_tags(text)).strip()
    for field, value in {
        "센터명": center,
        "센터 주소": address,
        "교육지원청명칭": office,
        ...
        if value and re.sub(r"\s+", " ", value).strip() not in normalized_text:
    ...
    if config.grade in grades:
        if config.grade not in text or not re.search(r"자료에\s*기재|가능\s*학년|수강\s*가능", text):
            errors.append(f"{local}: {config.grade} {config.subject} 가능학년 사실 표시 누락")
    elif "상담 확인 필요" not in text:
        errors.append(f"{local}: {config.grade} {config.subject} 가능학년 미기재 지역의 상담 확인 안내 누락")
...
def run_audit(config: AuditConfig) -> int:
    errors: list[str] = []
    warnings: list[str] = []
    hub = SUBJECT_ROOT / config.category
    previous_hub = SUBJECT_ROOT / config.previous_category
    ...
    expected_urls = {shared.normalize_url(canonical(config, local)) for local in locals_}
    if hub.exists():
        actual_dirs = {
        ...
        actual_dirs = set()
        hub_text = ""
        errors.append(f"{config.label} 허브 폴더 없음: {hub}")
    ...
    hub_url = canonical(config)
    card_urls: list[str] = []
    if hub_text:
        verify_head(hub_text, config.label, hub_url, "허브", errors)
        expected_hub_breadcrumb = ["홈", "과목별학원", config.label]
    ...
    for local in locals_:
        ...
        url = shared.normalize_url(canonical(config, local))
        ...
        if missing_sections:
            errors.append(f"{local}: {config.label} 필수 섹션 누락 ({missing_sections})")
        ...
        expected_breadcrumb = ["홈", "과목별학원", config.label, expected_title]
        ...
        if len(screen_faq) != config.faq_count:
            errors.append(f"{local}: 화면 FAQ가 {config.faq_count}개가 아님 ({len(screen_faq)}개)")
        ...
        previous_url = shared.normalize_url(previous_canonical(config, local))
        if previous_url not in visible_related:
            errors.append(f"{local}: 같은 동네 {config.previous_label} 관련 링크 누락")
        if not (previous_hub / local / "index.html").exists():
            errors.append(f"{local}: {config.previous_label} 링크 대상 페이지 없음")
        ...
        verify_facts(config, row, local, text, errors)

        manuscript_raw = shared.manuscript_html(text)
        body = shared.strip_tags(manuscript_raw)
        if len(body) < config.min_manuscript_length:
            errors.append(
                f"{local}: {config.label} 원고·보강 영역이 너무 짧음 "
                f"({len(body)}자, 최소 {config.min_manuscript_length})"
            )
        for intent, pattern in config.intent_groups.items():
            if not pattern.search(body):
                errors.append(f"{local}: {config.label} 학습 의도 필수 요소 누락 ({intent})")
        for label, pattern in config.forbidden_patterns.items():
    ...
    if missing_sitemap:
        errors.append(f"{config.label} 사이트맵 URL 누락 {len(missing_sitemap)}개")
...
if __name__ == "__main__":
    sys.exit(run_audit(ENGLISH_CONFIG))
