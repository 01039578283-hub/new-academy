...
import audit_middle3_math_pages as shared


ROOT = Path(__file__).resolve().parents[1]
DATA = ROOT.parent / "참고자료" / "공통자료" / "센터정보 정리.csv"
SUBJECT_ROOT = ROOT / "과목별학원"
HUB = SUBJECT_ROOT / "고1수학학원"
PREVIOUS_HUB = SUBJECT_ROOT / "중3수학학원"
...
HIGH1_MATH_INTENT_GROUPS = {
    "고교 첫 내신 적응": re.compile(r"고1|고등학교\s*1학년|고교\s*(?:첫\s*)?내신|첫\s*내신|고등\s*내신"),
    "공통수학 개념 연결": re.compile(
        r"공통수학|다항식|방정식|부등식|경우의\s*수|행렬|도형의\s*방정식|집합과\s*명제|함수"
    ),
    "풀이 과정": re.compile(r"풀이\s*(?:과정|순서|근거)|식\s*전개|조건\s*해석|검산"),
    "서술형": re.compile(r"서술형|풀이\s*근거|답안\s*작성|과정형\s*문항"),
    "오답 재학습": re.compile(r"오답\s*(?:재학습|관리|기록)|재풀이|다시\s*풀|오류\s*원인"),
    "모의평가": re.compile(r"모의평가|모의고사|전국연합|학력평가"),
    "학습 시간관리": re.compile(
...
FORBIDDEN_PATTERNS = {
    ...
    "영어 중심 용어": re.compile(r"영문법|영어\s*단어|영어\s*독해|영작|구문\s*독해|교과서\s*본문\s*암기"),
...
def verify_head(text: str, title: str, url: str, label: str, errors: list[str]) -> None:
    ...
    og_urls = [
        shared.normalize_url(value, url)
        for value in shared.meta_value(text, "og:url", "property")
        if value
    ]
...
def verify_facts(row: dict[str, str], local: str, text: str, errors: list[str]) -> None:
    ...
    if "고1" in grades:
        if "고1" not in text or not re.search(r"자료에\s*기재|가능\s*학년|수강\s*가능", text):
            errors.append(f"{local}: 고1 수학 가능학년 사실 표시 누락")
    elif "상담 확인 필요" not in text:
        errors.append(f"{local}: 고1 수학 가능학년 미기재 지역의 상담 확인 안내 누락")
...
def main() -> int:
    ...
    if HUB.exists():
        ...
        actual_dirs = set()
        hub_text = ""
        errors.append(f"고1 수학 허브 폴더 없음: {HUB}")
    ...
    for local in locals_:
        ...
        if missing_sections:
            errors.append(f"{local}: 고1 수학 상세 필수 섹션 누락 ({missing_sections})")

        descriptions = shared.meta_value(text, "description")
        ...
        if previous_url not in visible_related:
            errors.append(f"{local}: 같은 동네 중3 수학학원 관련 링크 누락")
        if not (PREVIOUS_HUB / local / "index.html").exists():
            errors.append(f"{local}: 중3 수학학원 링크 대상 페이지 없음")
        ...
        if len(body) < 1900:
            errors.append(f"{local}: 고1 수학 원고·보강 영역이 너무 짧음 ({len(body)}자)")
        for intent, pattern in HIGH1_MATH_INTENT_GROUPS.items():
            if not pattern.search(body):
                errors.append(f"{local}: 고1 수학 학습 의도 필수 요소 누락 ({intent})")
    ...
    if missing_sitemap:
        errors.append(f"고1 수학 사이트맵 URL 누락 {len(missing_sitemap)}개")
